...
- **ADR 0002 — Runtime versions fixed.** Hiagent = `2.6`; Dify = `1.14.0`. These are the v1 pins unless an explicit compatibility ADR changes them.
...
- **Runtime import/export canonicalization proven on the Phase 0 engineering target (Dify).** Take a hand-authored IR, compile to Dify DSL via the Dify adapter shim, push to the pinned Dify, pull back, run Dify's canonicalization (§6.4), confirm the canonical AST hash is stable across N=10 round-trips. False-drift rate must be 0. ADR 0002 also pins Hiagent (a *decision* artifact); the Hiagent canonicalization proof is a Phase 1 Task 11.5 deliverable, not Phase 0. If the Cost-budget escape hatch was invoked **before** Phase 0 completes and Dify was dropped, this row's proof is performed against Hiagent instead.
...
- **One Reviewer edit simulation on the Phase 0 engineering target (Dify).** An internal reviewer, real partner reviewer, or synthetic SOW reviewer opens a generated workflow in Dify, makes realistic edits, and walks through the publish-blocking + remediation flow.
...
- **Runtime DSL drift between versions** (per Hiagent / Dify) → Pin each runtime's version (ADR 0002); have a single compiler module per runtime major version (`loom/runtimes/<target>/v<X_Y>/`); CI runs golden tests against each pinned runtime. Each runtime version bump is treated as an explicit compatibility project on that runtime (re-run conformance matrix end-to-end + parity test), not a routine dependency update.
- **Conformance-test flake** → The semantic conformance suite (§5) runs against live runtime instances (Hiagent + Dify in v1). Network/timing/runtime nondeterminism produces flakes. Mitigation: pin each runtime to a specific docker tag + digest in CI (per ADR 0002); report flake rate per runtime as a metric (§10); a flake rate >5% on any runtime blocks a release as much as a red cell.
...
- **Vendor lock to pinned runtime versions (Hiagent + Dify)** → Pinning avoids surprise breakage but creates a slow upgrade path. Mitigation: portability is by construction — the IR compiles to two runtimes from Day 1 (Phase 1). If a runtime forces a version upgrade and our conformance suite goes red, we route customers to self-hosted on the pinned version while we close the gap, OR to the other runtime if the affected workflows have parity coverage. Phase 1.5 runtime parity contract is the regression guard.
